...
export default function Calendar() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setEvents((data.events || []).map((e) => ({ ...e, start: new Date(e.start), end: new Date(e.end) })));
    } catch (e) {}
  }, [range.from, range.to]);
  ...
  const segBtn = (v, txt) =>
    `px-3 py-1.5 text-sm font-semibold rounded-lg transition-colors ${
      view === v ? "bg-primary text-on-primary" : "text-on-surface-variant hover:bg-surface-variant"
    }`;
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <p className="text-base text-on-surface-variant">Bookings across both study rooms.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="ml-auto flex items-center gap-1 bg-surface-container-lowest rounded-xl p-1 border border-solid border-outline-variant/60">
          <button onClick={() => setView("month")} className={segBtn("month")}>Month</button>
          <button onClick={() => setView("week")} className={segBtn("week")}>Week</button>
          <button onClick={() => setView("day")} className={segBtn("day")}>Day</button>
        </div>
      </div>
      ...
      {view === "month" && <MonthView cursor={cursor} events={events} onPick={setSel} />}
      {view !== "month" && (
        <TimeGrid days={view === "week" ? Array.from({ length: 7 }, (_, i) => addDays(startOfWeek(cursor), i)) : [startOfDay(cursor)]} events={events} onPick={setSel} />
      )}

      {sel && <EventDetail e={sel} onClose={() => setSel(null)} />}
    </>
  );
}

/* ---- Month grid ---- */
function MonthView({ cursor, events, onPick }) {
    ...
    <div className="bg-surface-container-lowest rounded-2xl ambient-shadow border border-solid border-outline-variant/40 overflow-hidden">
      ...
      <div className="grid grid-cols-7">
        {cells.map((day, i) => {
          ...
          const dayEvents = events.filter((e) => e.start < addDays(day, 1) && e.end > day).sort((a, b) => a.start - b.start);
          return (
            <div key={i} className={`min-h-[92px] border-b border-r border-solid border-outline-variant/30 p-1.5 ${inMonth ? "" : "bg-surface-container-low/40"}`}>
              ...
              <div className="flex flex-col gap-1">
                {dayEvents.slice(0, 3).map((e) => (
                  <button key={e.id} onClick={() => onPick(e)} className="w-full text-left text-[11px] leading-tight px-1.5 py-1 rounded-md truncate" style={{ background: ROOM[e.room].bg, color: ROOM[e.room].text }}>
                    <span className="font-semibold">{fmtTime(e.start)}</span> {e.title}
                  </button>
                ))}
                {dayEvents.length > 3 && (
                  <span className="text-[10px] text-on-surface-variant pl-1">+{dayEvents.length - 3} more</span>
                )}
              </div>
            </div>
          );
...
/* ---- Week / Day time grid ---- */
function TimeGrid({ days, events, onPick }) {
    ...
    <div className="bg-surface-container-lowest rounded-2xl ambient-shadow border border-solid border-outline-variant/40 overflow-x-auto">
      <div className="min-w-[560px]">
        ...
        <div className="grid" style={{ gridTemplateColumns: `56px repeat(${days.length}, 1fr)` }}>
          ...
          {/* day columns */}
          {days.map((day, di) => {
            const dayEvents = events.filter((e) => sameDay(e.start, day) || (e.start < day && e.end > day));
            return (
              <div key={di} className="relative border-l border-solid border-outline-variant/30" style={{ height: hours.length * HOUR_PX }}>
                {hours.map((h) => (
                  <div key={h} className="border-b border-solid border-outline-variant/20" style={{ height: HOUR_PX }} />
                ))}
                {dayEvents.map((e) => {
                  ...
                  return (
                    <button key={e.id} onClick={() => onPick(e)} className="absolute left-1 right-1 rounded-md px-1.5 py-1 text-left overflow-hidden" style={{ top, height, background: ROOM[e.room].bg, borderLeft: `3px solid ${ROOM[e.room].border}`, color: ROOM[e.room].text }}>
                      <div className="text-[11px] font-semibold leading-tight truncate">{e.title}</div>
                      <div className="text-[10px] leading-tight opacity-80">{fmtTime(e.start)}–{fmtTime(e.end)}</div>
                    </button>
                  );
                })}
              </div>
    ...
    <div className="fixed inset-0 z-[60] bg-on-background/40 flex items-end md:items-center justify-center p-0 md:p-6" onClick={onClose}>
      <div className="bg-surface-container-lowest w-full md:max-w-md rounded-t-2xl md:rounded-2xl ambient-shadow p-6" onClick={(ev) => ev.stopPropagation()}>
        <div className="flex items-start justify-between mb-2">
          <span className="text-xs font-bold tracking-wide uppercase px-2.5 py-1 rounded-full" style={{ background: ROOM[e.room].bg, color: ROOM[e.room].text }}>Study Room {e.room}</span>
        ...
        <p className="text-sm text-on-surface-variant mt-1 flex items-center gap-1.5">
          ...
          {new Date(e.start).toLocaleString([], { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })} – {fmtTime(e.end)}
        </p>
      </div>
    </div>
  );
